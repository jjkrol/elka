...
/* get process index in the process table with process id given */
int do_getprocnr(void){
int proc_number; /*loop variable*/
for(proc_number=0; proc_number<NR_PROCS; proc_number++){ /*iterate over the process table*/
if(mproc[proc_number].mp_flags & IN_USE !=0) /* only active processes are interesting */
if(mproc[proc_number].mp_pid == pid) /* check if we found that process */ 
return proc_number; /*return the found number*/
}
return ENOENT; /*no process was found */
}
