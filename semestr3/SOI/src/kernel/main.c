...
/* Start the ball rolling. */
  ...
  for (rp = BEG_PROC_ADDR, t = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++t) {
	...
	rp->cur_pri = MIN_PRI; /* initialize cur_pri */
	rp->base_pri = MIN_PRI; /* initialize base_pri */
	rp->group_type = INTERAKTYWNY; /* initialize group type */
  ...
  proc[NR_TASKS+INIT_PROC_NR].cur_pri = MIN_PRI; /* actually doesn't matter */
  proc[NR_TASKS+INIT_PROC_NR].base_pri = MIN_PRI; /* because init is INTERAKTYWNY */
  proc[NR_TASKS+INIT_PROC_NR].group_type = INTERAKTYWNY;
