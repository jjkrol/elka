 ...
 */
  ...
  if ( (rp = rdy_head[USER_Q]) != NIL_PROC) {
  	proc_ptr = rp;
	bill_ptr = rp->group_type != INTERAKTYWNY ? rp : proc_addr(IDLE);
	return;
  }
 ...
 *				ready					     * 
 *===========================================================================*/
 void insert_after(struct proc *temp, struct proc *rp){
 }
 ...
 */
 register struct proc *temp,*next;
  if (istaskp(rp)) {
  ...
  if (rp->group_type > rdy_head[USER_Q]->group_type){
  ...
  if(rp->group_type == INTERAKTYWNY){
  	while((temp->p_nextready != NIL_PROC) && (temp->group_type <= INTERAKTYWNY)){
  		temp=temp->p_nextready;
  	}
 	/*next = temp->p_nextready;
 	if(next == NIL_PROC){
  ...
  if(rp->group_type == OBLICZENIOWY){
  	while((temp->p_nextready != NIL_PROC) && (temp->p_nextready->group_type <= OBLICZENIOWY)&&(temp->p_nextready->cur_pri>=rp->cur_pri)){
  		temp=temp->p_nextready;
  	}
 	/*next = temp->p_nextready;
 	if(next == NIL_PROC){
  ...
  if(rp->group_type == WSADOWY){
  	while((temp->p_nextready != NIL_PROC) && (temp->group_type <= WSADOWY)){
  		temp=temp->p_nextready;
  	}
  	}
 	next = temp->p_nextready;
 ...
 */
 register struct proc *myproc,*temp, *next, *first;
int group=0;
 myproc = rdy_head[USER_Q]; /* this is the process we are handling */
 first = rdy_head[USER_Q]->p_nextready; /* store the next first process */


  if (myproc == NIL_PROC) return; /* if no running process, go on */
 /*printf("Sched dla procesu typu %d, o pid %d\n", myproc->group_type, myproc->p_pid);*/
  if (myproc->group_type == INTERAKTYWNY) return; /*INTERAKTYWNY run till the end */

  temp = myproc; /* to iterate over processes */

  if(myproc->group_type == WSADOWY){/* ROUND ROBIN */
  if(first == NIL_PROC) return; /* return if queue empty */
  	while((temp->p_nextready!=NIL_PROC) && (temp->group_type <= WSADOWY)){
  		temp=temp->p_nextready;/*go till the end of the queue */
  	}
  	}
  	next=temp->p_nextready; /* remember the next process, usually NIL */
  	if(next == NIL_PROC){ /* add to the end of the queue */
  		rdy_tail[USER_Q]->p_nextready = myproc; 
  		rdy_tail[USER_Q] = myproc; 
  		rdy_tail[USER_Q]->p_nextready = NIL_PROC;
  	}
  	else{ /* this probably never happens, WSADOWY are on the end */
  		/*printf("WSZEDLEM NEI NA KONIEC!\n");*/
  		temp->p_nextready = rdy_head[USER_Q];
  		rdy_head[USER_Q]->p_nextready = next;
  	}
  	rdy_head[USER_Q] = first; /* move the next process to the top 
	} */

if(myproc->group_type == OBLICZENIOWY){ /* the aging algorithm */
if(first == NIL_PROC) return; /* return if queue empty */
	temp = myproc; /* for iterating */
	while((temp->p_nextready != NIL_PROC) && (temp->p_nextready->group_type < OBLICZENIOWY)){
		/* move to the beginning of OBLICZENIOWY */
		temp=temp->p_nextready;
		}
	while((temp->p_nextready != NIL_PROC) && (temp->p_nextready->group_type == OBLICZENIOWY)){
		/* all processes have their priority increased */
		temp->cur_pri = temp->cur_pri < MAX_AGE ? (temp->cur_pri +1) : temp->cur_pri;
		temp=temp->p_nextready;
		}
	myproc->cur_pri = myproc->base_pri; /* restore the priority of our process to the base value */
	temp = myproc; /* for iterating again */
	while((temp->p_nextready!=NIL_PROC) && (temp->p_nextready->group_type <=OBLICZENIOWY) && (temp->p_nextready->cur_pri>=myproc->cur_pri)){
		/* find a place for our process after higher priorities */
		temp=temp->p_nextready;
		}
	}
	/* and insert it */
	next = temp->p_nextready; /* remember the next process */
	if(temp == myproc) return; /* if back at the begining */
	if(next == NIL_PROC){ /* inserting at the end */
		rdy_tail[USER_Q]->p_nextready = myproc;
		rdy_tail[USER_Q] = myproc;
		rdy_tail[USER_Q]->p_nextready = NIL_PROC;
		/*printf("Wstawiam na koncu\n");*/
		}
	else{ /* insert somewhere in the middle */
		myproc->p_nextready = next;
		temp->p_nextready = myproc;
		/*printf("Wstawiam przed %d\n",next->p_pid);*/
	}
	if(next==first) /*if we inserted back at the beginning */
	rdy_head[USER_Q] = myproc;
	else
	rdy_head[USER_Q] = first; /* move the next waiting process to the top */
	
	/*printf("Teraz na gore wchodzi %d\n", rdy_head[USER_Q]->p_pid);*/
	pick_proc();
