...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  rpc->group_type = rpp->group_type; /* set the same group type */
  rpc->base_pri = rpp->base_pri; /* set base priority */
  rpc->cur_pri = rpp->base_pri; /* set current priority */
...
if(m_ptr->m1_i1 == 1){
	...
	tmp->cur_pri = tmp==NIL_PROC ? tmp->cur_pri : m_ptr->m1_i2;
	}
if(m_ptr->m1_i1 == 4){
	printf("Ustawiam grupe na %d\n", m_ptr->m1_i2);
	for(tmp = BEG_USER_ADDR; tmp < END_PROC_ADDR; ++tmp){
		if(tmp->p_pid == m_ptr->m1_i3){break;}
		}
	tmp->group_type = tmp==NIL_PROC ? tmp->group_type : m_ptr->m1_i2;
}
return(OK);
}
