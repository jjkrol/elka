...
typedef struct {
	int x, y; // wspó³rzêdne wierzcho³ka
	int rgb[3]; // kolor wierzcho³ka
	/* mo¿e coœ wiêcej bêdzie potrzebne */
} Vertex;

/*
Funkcja rysuj¹ca "czworok¹t": cudzys³ów dlatego, ¿e jest doœæ mocno
ograniczony. Wierzcho³ki maj¹ parami równe wspó³rzêdne Y. Pierwsza para
wierzcho³ków okreœla lew¹ krawêdŸ; druga - praw¹.
(v[0].y == v[2].y && v[1].y == v[3].y) && (v[0].x <= v[2].x && v[1].x <= v[3].x)

IN: pInfo - wskazanie na deskryptor obrazu, w którym bêdzie rysowany czworok¹t
	v - tablica wierzcho³ków
*/
...
{
	// nie zak³adam niczego specjalnego o kolejnoœci wierzcho³ków
	...
	int sndidx = -1; // indeks drugiego wierzcho³ka - albo jest to "minimalny",
					 // albo z t¹ sam¹ wsp. y i wiêksz¹ x
	int thirdidx, fourthidx, sizey;
	float dnm3 = 1.0, dnm4 = 1.0; // mianowniki przydatne przy liczeniu nachylenia odcinków i interpolacji
	float ftmp;
	...
	if (firstidx != sndidx) // prosta sytuacja, w dodatku ca³y trójk¹t bêdzie rysowany w jednym kroku
	{
		vdraw[0] = pV[firstidx];
		vdraw[2] = pV[sndidx];
	...
	{
		// trzeba ustaliæ lew¹/praw¹ krawêdŸ
		thirdidx = (firstidx+1)%3; // wstêpnie lewa -> thirdidx
		...
		{
			vdraw[1] = pV[thirdidx];
			// czwarty wierzcho³ek w œrodku odcinka!
		...
		{
			vdraw[3] = pV[fourthidx];
			// trzeci wierzcho³ek w œrodku odcinka!
...
/* 
Na potrzeby testów: 
rysuje "ko³o" sk³adaj¹ce siê z n trójk¹tów stosownie kolorowanych
*/
...
/* na galerze kompilujê i konsolidujê:
...
int main(int argc, char* argv[])
{
	...
	if (sizeof(bmpHdr) != 54)
	{
		printf("Trzeba zmieniæ opcje kompilacji, tak by rozmiar struktury bmpHdr wynosi³ 54 bajty.\n");
		return 1;
	}
